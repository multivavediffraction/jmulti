...
import org.jmulti.{CalcParams, Logger, UnitCell}
...
class Calc {
  ...
  def apply(uc:UnitCell, atoms:Array[AtomDescr], params: CalcParams): Unit = {
    ...
    val filename = s"${params.title}-(${params.h}${params.k}${params.l})-E ${params.energy}-psi ${params.psiStart} ${params.psiEnd} ${params.psiSteps}"
    ...
    val r0:Double = 2.81794092e-5d //classical electron radius in Angstrem

    val lambda=12.398519/params.energy // calculated lambda in A, Ex(keV) was read above
    ...
    outWriter.println(f"$thetab%18.8f    - Bragg angle thetaB (grad)")
    outWriter.println(f"$lambda%18.8f    - wavelength (angstrom)")
    outWriter.println(f"${params.energy}%18.8f    - Ex- x-ray energy")
    outWriter.println()
...
//        if p.normSquare >= 1.0E1 // was 5 this excludes small umweg reflections
    ...
    val azimLin = new PrintWriter(s"azim-lin-$filename.csv") // 2
    val azimCirc = new PrintWriter(s"azim-circ-$filename.csv") //3

    val ncount = factors.length  // counts the number of reflection pairs with nonzero F*F

    azimLin.println(s"Psi;fMod2ss;fMod2pp;fMod2ps;fMod2sp;fMod2s;fMod2p;fMulssRe;fMulssIm;fMulppRe;fMulppIm;fMulpsRe;fMulpsIm;fMulspRe;fMulspIm;fQQRe;fQQIm;fDQRe;fDQIm;fMagRe;fMagIm;fDQ=$fDQ fQQ=$fQQ ncount=$ncount Energy=${params.energy}")

    azimCirc.println(s"Psi;Rr2;Rl2;(Rr2-Rl2)/(Rr2+Rl2);(Rr2+Rl2)/2;ncount=$ncount, Energy=${params.energy}")

//    val peaks = new PrintWriter("peaks_hkl.dat2") // 10
//    peaks.println("Psi h1 k1 l1 h2 k2 l2 Ffactor")
